...
<body class="bg-white flex justify-center items-center min-h-screen p-2">
    <div class="max-w-sm w-full bg-indigo-800 p-6 rounded-lg text-center text-white shadow-lg">
        ...
        <p class="text-xs mt-2 px-2" id="welcomeinfo" >
            <?php if ($guest) { ?>  <b>Welcome back {{ $guest['name'] }}</b><?php } else {?>By clicking the "Fill Form" button, you consent to receive marketing, promotional messages and information about Kuta Social Club and its affiliated hotel network (Ovolo, By Ovolo Collective). You may opt out of communications at any time. All data obtained is subject to the Privacy Policy.<?php } ?>
        </p>
    ...
    <div id="modal" class="fixed inset-0 flex items-center justify-center bg-gray-700 bg-opacity-50 hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <input type="hidden" id="useragent" value="{{$data['useragent']}}"/>
    ...
    <script>
        ...
        // --- MODIFIED submit handler: setelah backend buat username/password, langsung login (tanpa welcome) ---
        submitForm.addEventListener('click', async function(e) {
            ...
            try {
                ...
                if (result.error === true) {
                    // tampilkan error validasi (tetap di modal)
                    let errmsg = "";
                    ...
                    // tetap tutup modal? biarkan modal terbuka agar user bisa perbaiki
                    // modal.classList.remove('hidden'); // biarkan modal tetap terbuka
                } else {
                    // berhasil: set username & password dari sistem lalu langsung submit form login ke hotspot
                    ...
                    // --- Jika hotspot memakai CHAP, hash password sebelum submit ---
                    <?php if(strlen($data['chap-id']) < 1)  { ?>
                        // plain submit
                        document.loginweb.submit();
                        return false;
                    <?php }  else { ?>
                        // hash menggunakan hexMD5 dari md5.js: hexMD5(chap-id + password + chap-challenge)
                        // pastikan hexMD5 tersedia (file /md5.js)
                        try {
                            const plainPwd = document.loginweb.password.value;
                            const chapId = '<?php echo $data['chap-id']; ?>';
                            const chapChallenge = '<?php echo $data['chap-challenge']; ?>';
                            document.loginweb.password.value = hexMD5(chapId + plainPwd + chapChallenge);
                        } catch (err) {
                            console.error('Error saat hashing CHAP:', err);
                        }
                        document.loginweb.submit();
                        return false;
                    <?php } ?>
                ...
                console.log(error);
                errorInfo.innerHTML = 'Terjadi kesalahan, coba lagi.';
            }
        });

        // tetap pertahankan tombol Sign-in asli (jika pengguna ingin manual sign-in)
