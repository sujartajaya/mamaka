...
<body class="bg-white flex justify-center items-center min-h-screen p-2">
    <div class="max-w-sm w-full bg-indigo-800 p-6 rounded-lg text-center text-white shadow-lg">
        ...
        <p class="text-xs mt-2 px-2" id="welcomeinfo" >By clicking the "Fill Form" button, you consent to receive marketing, promotional messages and information about Kuta Social Club and its affiliated hotel network (Ovolo, By Ovolo Collective). You may opt out of communications at any time. All data obtained is subject to the Privacy Policy.</p>

        <button id="openModal" class="bg-teal-400 text-indigo-900 font-bold px-6 py-2 rounded-lg mt-4 w-full">Fill Form</button>
        <button id="siginForm" class="bg-teal-400 text-indigo-900 font-bold px-6 py-2 rounded-lg mt-4 w-full hidden">Sigin</button>

        <div class="mt-6 text-sm">
            <p class="font-bold text-red-800">{{ $data['error'] }}</p>
        </div>
        <div class="mt-6 text-sm">
    ...
    <script>
        ...
        submitForm.addEventListener('click', async function(e) {
            ...
            // document.register.name.value = name.value;
            // document.register.email.value = email.value;
            // document.register.country_id.value = option.value;
